...
p1 <- proportion %>% 
  ...
  geom_bar(aes(y=(..count..)/sum(..count..)),width = .1,colour="#1D0D6E")+
  scale_y_continuous(labels=scales::percent,limits = c(0,.5))+
  labs(x="Period of the day",
       y="Relative frequencies",
       title = "Incident per period of day with damage distinction",
       subtitle = glue("{nonedmg} % of incident implies none damage. Good News!!! Also, 37,25 % of incident happens during the day and implied none damage!\n\n"),
       fill="",
       caption = "Author: @alangel12407606 | #TidyTuesday")+
  geom_label(aes( label = paste(round(100*(..count..)/sum(..count..),digits = 2),"%"),
                  y= (..count..)/sum(..count..) ), stat= "count", vjust =-1.5,size=4,show.legend = F)+
  ...
  theme(legend.position = c(0.85,0.15),
        ...
        panel.background = element_rect(fill = "#F0FEFF",colour = "black"),
        #plot.background = element_rect(fill="#FDFFE3"),
...
p1 <- ggdraw(plot = p1)+draw_image(image = "C:/Users/alain/Pictures/hourglass.png",scale = .07,y = -0.19,x=0.35)
...
incident_number_year <- wildlife_impacts %>% 
  group_by(incident_year) %>% 
  tally() %>% 
  mutate(incident_year=lubridate::ymd(incident_year, truncated = 2L))

p2 <- incident_number_year %>% 
  ggplot(aes(x=incident_year,y = n))+
  geom_line(size=1.1)+
  geom_point(size=3,colour="#C41616")+
  scale_x_date(date_breaks = "1 year",date_labels ="%Y")+
  theme_minimal()+
  theme(panel.grid.minor = element_blank())+
  labs(x="",
       y="# of incidents",
       title = "Incident by year")
...
```{r echo=FALSE, fig.height=5, fig.width=10}
incident_number_month <- wildlife_impacts %>% 
  group_by(incident_year,incident_month) %>% 
  tally() %>% 
  unite(col = "incident_number_year_month",c("incident_year","incident_month"),sep = "-") %>% 
  mutate(incident_number_year_month=lubridate::ymd(incident_number_year_month, truncated = 1L))


p3 <- incident_number_month  %>% 
  ggplot(aes(x=incident_number_year_month,y = n))+
  geom_line(size=1.1)+
  scale_x_date(date_breaks = "1 year",date_labels ="%Y")+
  theme_minimal()+
  theme(panel.grid.minor = element_blank())+
